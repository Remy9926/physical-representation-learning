...
class BaseFinetuner(Trainer, ABC):
    def __init__(self, cfg, trained_model_path=None, rank=0, world_size=1):
        ...
        assert self.cfg.dataset.name in STATS, (
            f"label stats for {self.cfg.dataset.name} not found"
        )
    ...
    def train(self):
        run_name = self.cfg.ft.get(
            "run_name",
            f"{self.cfg.dataset.name}-{self.cfg.dataset.num_frames}frames-{self.cfg.model.objective + '-FT' if not self.cfg.ft.get('not_from_embeddings', False) else 'supervised'}-{self.cfg.ft.task}{f'-randominit' if self.trained_model_path is None else ''}",
        )
        ...
        if self.world_size > 1:
            model_components[-1] = DDP(
                model_components[-1].to(self.rank), device_ids=[self.rank]
            )
        ...
        if self.cfg.ft.task == "binary_classification":
            assert self.cfg.ft.num_classes == 1, (
                "binary classification must have 1 class"
            )
        if self.cfg.ft.task == "classification" and self.cfg.ft.num_classes == 1:
            self.cfg.ft.task = "binary_classification"
        loss_fn = (
            self.loss_for_task[self.cfg.ft.task]
            if self.cfg.ft.task in self.loss_for_task
            else None
        )
        if loss_fn is None:
    ...
    def get_embeddings(self):
        ...
        runpath = (
            ...
            if self.trained_model_path is not None
                ...
                f"{self.cfg.dataset.name}-{self.cfg.dataset.num_frames}frames-{str(self.cfg.ft.get('fields', None)) + 'fields-' if self.cfg.ft.get('fields', None) is not None else ''}-{self.cfg.model.objective}-{self.cfg.ft.task}/randominit-seed{self.seed}"
            )
        )
        if (
            runpath.parent.name == "checkpoints"
        ):  # backwards compatibility w/ old model paths that weren't nested
            train_embeddings_path = (
                Path(self.cfg.ft.embeddings_dir) / f"{runpath.name}_embeddings_train.h5"
            )
            val_embeddings_path = (
                Path(self.cfg.ft.embeddings_dir) / f"{runpath.name}_embeddings_val.h5"
            )
        else:
            train_embeddings_path = (
                Path(self.cfg.ft.embeddings_dir)
                / f"{runpath.parent.name}_{runpath.name}_{'noise-' + str(self.cfg.ft.get('noise_std', 0.0)) + '_' if self.cfg.ft.get('noise_std', 0.0) > 0.0 else ''}embeddings_train.h5"
            )
            val_embeddings_path = (
                Path(self.cfg.ft.embeddings_dir)
                / f"{runpath.parent.name}_{runpath.name}_{'noise-' + str(self.cfg.ft.get('noise_std', 0.0)) + '_' if self.cfg.ft.get('noise_std', 0.0) > 0.0 else ''}embeddings_val.h5"
            )

        self.cfg.ft.embeddings_path = str(train_embeddings_path)
        if not train_embeddings_path.exists():
            print(
                f"no embeddings found at {train_embeddings_path}, calculating",
                flush=True,
            )

            encoder = self.get_encoder_and_raw_loaders()
            train_batches_per_epoch = min(
                self.train_cfg.num_train_steps
                if "num_train_steps" in self.train_cfg
            ...
            with h5py.File(train_embeddings_path, "w") as train_file:
                ...
                train_dataset_size = (
                    train_batches_per_epoch * self.train_cfg.batch_size
                    if "num_train_steps" in self.train_cfg
                    else len(self.train_loader.dataset)
                )
                if not isinstance(self.train_loader.dataset, IterableDataset):
                    assert train_dataset_size <= len(self.train_loader.dataset), (
                        "num_train_examples is greater than the size of the train dataset"
                    )
                for i, batch in tqdm(
                    ...
                    if len(train_embeddings_buffer) >= buffer_size:
                        ...
                        train_file["embeddings"][start_idx:end_idx] = (
                        ...
                        buffer_memory = (
                            sum(arr.nbytes for arr in train_embeddings_buffer)
                            / 1024
                            / 1024
                            / 1024
                        )  # GB
                        print(
                            f"Buffer written at step {i}: Memory: {current_memory:.1f} GB, Buffer size: {buffer_memory:.1f} GB",
                            flush=True,
                        )
                    ...
                    if i % 100 == 0 and i > 0:
                        ...
                        print(
                            f"Step {i}: Memory: {current_memory:.1f} GB, Buffer: {buffer_memory:.1f} GB, Buffer count: {len(train_embeddings_buffer)}",
                            flush=True,
                        )
                ...
                print(
                    f"num train embeddings: {total_train_count}, size of each embedding: {train_file['embeddings'].shape[1:]}"
                )
        ...
        if not val_embeddings_path.exists():
            ...
            val_batches_per_epoch = (
                self.train_cfg.num_val_steps
                if "num_val_steps" in self.train_cfg
            ...
            with h5py.File(val_embeddings_path, "w") as val_file:
                ...
                for i, batch in tqdm(
                    ...
                    if len(val_embeddings_buffer) >= buffer_size:
                        ...
                        buffer_memory = (
                            sum(arr.nbytes for arr in val_embeddings_buffer)
                            / 1024
                            / 1024
                            / 1024
                        )  # GB
                        print(
                            f"Val buffer written at step {i}: Memory: {current_memory:.1f} GB, Buffer size: {buffer_memory:.1f} GB",
                            flush=True,
                        )
                    ...
                    if i % 50 == 0 and i > 0:
                        ...
                        print(
                            f"Val step {i}: Memory: {current_memory:.1f} GB, Buffer: {buffer_memory:.1f} GB, Buffer count: {len(val_embeddings_buffer)}",
                            flush=True,
                        )
                ...
                print(
                    f"num val embeddings: {total_val_count}, size of each embedding: {val_file['embeddings'].shape[1:]}"
                )
...
class JepaFinetuner(BaseFinetuner):
    def load_model(self):
        encoder, _, _ = get_model_and_loss_cnn(
            ...
            if "fields" not in self.cfg.ft
            else len(self.cfg.ft.fields),
        )
        if self.trained_model_path is not None:
            ...
            print(
                f"no pretrained model path provided, randomly initializing encoder",
                flush=True,
            )
    ...
    def load_model(self):
        if self.trained_model_path is not None:
            model_config = json.load(
                open(Path(self.trained_model_path).parent / "config.json")
            )
    ...
    def create_head(self, metadata):
        ...
        if self.cfg.ft.get("use_attentive_pooling", False):
            ...
            if self.cfg.ft.task == "regression":
                ...
                raise ValueError(
                    f"task {self.cfg.ft.task} not supported for VideoMAE finetuning"
                )
        return head
    ...
    def _model_inference(self, ctx, encoder):
        with torch.no_grad():
            if self.cfg.ft.get("use_attentive_pooling", False):
                ...
                cls_token = encoder.forward_features(
                    ctx
                )  # (B, embed_dim) - already the CLS token
                return cls_token
